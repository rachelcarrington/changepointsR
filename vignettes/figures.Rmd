...
while( iter <= num_iter ){
  ...
  if ( length(results$changepoints) >= 1 ){ ## only keep simulations for which the changepoint algorithm detects at least one changepoint
    if ( results$changepoints[1] >= h & results$changepoints[1] <= (n-h) ){
      x <- calculate_pvals(y, results=results, N=50, h=h, sigma2=1, num_pvals=1, return_probs=TRUE)
...
while( iter <= num_iter ){
  ...
  if ( length(results$changepoints) >= 1 ){
    if ( results$changepoints[1] >= h & results$changepoints[1] <= (n-h) ){
      x <- calculate_pvals(y, results=results, N=50, h=h, sigma2=1, num_pvals=1, include_original=TRUE, return_probs=TRUE)
