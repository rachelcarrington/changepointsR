...
calculate_interval_not <- function(y, nu, results, nu2=NULL, nuTy=NULL){
  ...
  if ( nrow(results$results) == 0 ){
    ...
    for ( j in 1:nrow(rand_ints) ){  
      ...
      inequalities_list <- c(inequalities_list, (threshold - cs[,1]) / cs[,2], ((-1) * threshold - cs[,1]) / cs[,2])
      inequalities_signs <- c(inequalities_signs, ifelse(cs[,2] > 0, -1, 1), ifelse(cs[,2] > 0, 1, -1 ))
    }

  } else { # if >= 1 changepoints are detected
    # For narrower intervals, we need |C(t)| < threshold for all t in interval, discarding intervals containing previous CPs
    ...
    for ( k in 1:length(b) ){  
      ...
      if ( nrow(smaller_intervals) >= 1 ){
        for ( j in 1:nrow(smaller_intervals) ){
          ...
          inequalities_list <- c(inequalities_list, (threshold - cs2[,1]) / cs2[,2], ((-1) * threshold - cs2[,1]) / cs2[,2])
          inequalities_signs <- c(inequalities_signs, (-1) * (2 * (cs2[,2] > 0) - 1), 2 * (cs2[,2] > 0) - 1)
        }
      }
    ...
    # For interval containing the changepoint, we need |C(t)| < C(b1) for t \neq b1
        ...
        for ( j in 1:nrow(same_intervals) ){
          ...
          if ( sum(abs(interval - c(s[k], e[k]))) == 0 ){
            ...
            if ( abs(cs[b[k] - s[k] + 1, 2]) > 10^(-10) ){ # if denominator is 0, inequality is always satisfied so we can ignore it
              if ( d[k] == 1 ){
                inequalities_list <- c(inequalities_list, (-1)*(threshold + cs[b[k] - s[k] + 1, 1])/cs[b[k] - s[k] + 1, 2])
              } else {
                inequalities_list <- c(inequalities_list, (threshold - cs[b[k] - s[k] + 1, 1])/cs[b[k] - s[k] + 1, 2])
              }
