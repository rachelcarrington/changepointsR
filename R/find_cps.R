...
find_cps <- function(y, method, threshold=NULL, maxiter=NULL, num_rand_ints=NULL, rand_ints=NULL, seeded=FALSE, decay=NULL){
  ...
  ### y : vector of data
  ### method : "bs" = binary segmenation; "wbs" = wild binary segmentation; "not" = narrowest over threshold
  ### threshold : changepoint detection threshold for CUSUM statistic
  ### maxiter : max. number of changepoints to detect
  ### num_rand_ints : number of random intervals (ignored if rand_ints specified, or if method = "bs")
  ### rand_ints : random intervals for wild binary segmentation or narrowest over threshold (ignored if method = "bs")
  ### seeded : if TRUE and method = "wbs", then seeded binary segmentation is implemented
